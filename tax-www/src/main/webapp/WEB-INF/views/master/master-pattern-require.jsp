...
<body>
    ...
    <tilesx:useAttribute id="fullcontent" name="fullcontent" ignore="true" />
    <tilesx:useAttribute id="leftmenu" name="leftmenu" ignore="true" />
    <div id="page-container" class="fade ${!empty leftmenu ? "page-sidebar-fixed" : "page-without-sidebar"} page-header-fixed ${!empty fullcontent ? "page-content-full-height" : ""}">
        <!-- Header 시작 -->
        ...
        <!-- begin #content -->
        <div id="content" class="content ${!empty fullcontent ? "content-full-width" : ""}">
    ...
    <!-- GIS -->
    <c:if test="${!empty gisEngine}">
        <script type="text/javascript" src="${p}/res/plugins/openlayers-4.4.2/js/ol.js"></script>
        <%--<script type="text/javascript" src="${p}/res/plugins/gis/engine/${gisEngine}/${gisEngine}.js"></script>--%>
        <script type="text/javascript" src="${p}/res/plugins/gis/engine/${gisEngine}/mapInit.js"></script>

        <%--<c:choose>--%>
            <%--<c:when test="${gisEngine eq 'VWORLD'}">--%>
                <%--<script type="text/javascript" src="http://map.vworld.kr/js/apis.do?type=Base&apiKey=${gisApiKey}"></script>--%>
            <%--</c:when>--%>
        <%--</c:choose>--%>
    </c:if>
    ...
    <script>
        require(['common', 'darkhand', 'local', 'jquery'], function (common, darkhand, lc, jQuery) {
            ...
            jQuery(function () {
                ...
                // 왼쪽메뉴 URL 기준으로 활성화 기능 추가
                jQuery('#sidebar')
                    .find('li.active').removeClass('active').end()
                    .find('a').filter("[href='"+jQuery.fn.parseURL('').path+"']").parents('li, li.has-sub').addClass('active'); // li.has-sub
            });
        });
