...
define('local', ['common', 'formcheck', 'jqGrid.setting', 'jquery', 'jquery-ui', 'jqGrid'], function (common, formcheck, jqFn, jQuery) {
    ...
    function panelClear(isAll, objID, isListReset) {
        ...
        if (isAll) {
            // 모든 패널 초기화
            panelClear(false, 'configPanel', false);

            var rowid = jQuery("#configList").jqGrid('getGridParam', 'selrow');

            if (rowid == null && isListReset) {

                jQuery("#configList").jqGrid("resetSelection"); // Grid Select Reset 처리
            }
        ...
        switch (objID) {
            ...
            case 'configPanel':
                ...
                jQuery('#configList').jqGrid('clearGridData'); // 그리드 데이터 초기화
                break;
        }
    }
    ...
    function gridSetting(tarID) {
        ...
        switch (listID) {
            case 'configList' :
                ...
                jqOpt = {
                    url: '',
                    editurl: './setConfigAct',
                    scroll: 1,
                    scrollrows: true,
                    rowList: [10, 30, 50, 100],
                    colNames: ['','판별조건','수식','관리'],
                    colModel: [
                        {name: 'tcf_seq', index: 'tcf_seq', editable: true, hidden: true},
                        {name: 'filter_cd', index: 'filter_cd', width: 4, editable: true, edittype: 'select', sortable: false, editrules: { required: true },
                            editoptions: {
                                dataInit: function (el) {

                                    jQuery(el).attr({msg : '판별조건을'});
                                },
                                value: common.setjqGridOpt('-선택-', disConList)
                            }
                        },
                        {
                            name: 'fomula',
                            ...
                            formatoptions: {
                                ...
                                onEdit: function (rowid) {
                                    ...
                                    if (configSel !== rowid) {
                                        ...
                                        jQuery(this).jqGrid('restoreRow', configSel);
                                        jqFn.jqGridListIcon(this.id, configSel);
                                    }

                                    configSel = jQuery.jgrid.jqID(rowid);
                                },
                                ...
                                onSuccess: function (res) {
                                    // 저장 후 리턴 결과
                                    gridResAction(jQuery.parseJSON(res.responseText), 'config');
                                },
                                ...
                                delOptions: {
                                    ...
                                    serializeDelData: function () {
                                        ...
                                        var reqData = dataGrid.jqGrid('getRowData', configSel);
                                        return JSON.stringify({
                                            tcf_seq: reqData.tcf_seq
                                        });
                                    },
                    ...
                    onSelectRow: function (id, status, event) {
                        ...
                        if (id && id !== configSel) {
                            ...
                            if (configSel !== undefined) {

                                jQuery(this).jqGrid('restoreRow', configSel);
                                jqFn.jqGridListIcon(this.id, configSel);

                                // jqGrid 버그 수정 addRow 후에 다른 row 선택을 여러번 해보면 highlight 버그 해결
                                var tmpObj = jQuery(this).find('#' + configSel);
...
require(['common', 'darkhand', 'local', 'jquery'], function (common, darkhand, lc, jQuery) {
            ...
            case 0:
                ...
                tw.push({
                    ...
                    state: function () {
                        ...
                        lc.dataReload('config'); // 체납차량 판별조건 목록
    ...
    jQuery(function () {
        ...
        // 검색 폼
        common.setSelectOpt(jQuery('#srcDis'), '-전체-', disConList); // 판별조건

        // 체납차량 판별조건 목록 추가 버튼 클릭 이벤트
        jQuery('#btnAdd').on('click', function () {

            jQuery.when(
                jQuery('#configList').jqGrid('setGridParam', {page: 1})
            ).always(function () {
                    common.addRow('configList', {}, function () {
        ...
        jQuery("#configList").on("keydown", ':input', function (e) {

            if (e.keyCode === 13) {

                gridEnterSave('configList');
                return false;
            }
        });

        // 그리드 초기화
        lc.gridSetting('config'); // 체납차량 판별조건 목록
